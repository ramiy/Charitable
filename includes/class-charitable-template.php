...
class Charitable_Template {	
	...
	public function __construct($template_name, $load = false, $require_once = true) {
		$this->theme_template_path = trailingslashit( get_charitable()->get_path( 'theme_templates' ) );
	...
	private function locate_template() {
		 ...
		 */
		if ( ! $template ) {
			$template = get_charitable()->get_path( 'plugin_templates' ) . $this->template_names[0];
		}
