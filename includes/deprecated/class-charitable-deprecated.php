...
class Charitable_Deprecated {
     ...
     * @since   1.4.0
     */
    public function deprecated_argument( $function, $version, $alternatives = null ) {
        if ( ! $this->is_logging_enabled() ) {
            return false;
        }

        if ( ! is_null( $alternatives ) ) {
            $message = sprintf( __( '%1$s was called with an argument that is <strong>deprecated</strong> since version %2$s of Charitable! %3$s', 'charitable' ), $function, $version, $alternatives );
        }
        else {
    ...
    public function deprecated_function() {
        ...
        if ( ! is_null( $alternatives ) ) {
            $message = sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s of Charitable! Use %3$s instead.', 'charitable' ), $function, $version, $alternatives );
        }
        else {
