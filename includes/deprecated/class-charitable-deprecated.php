...
class Charitable_Deprecated {
     ...
     * @since   1.4.0
     */
    public function deprecated_function( $function, $version, $extra_message = null ) {
        if ( ! $this->is_logging_enabled() ) {
            return false;
        }

        if ( ! is_null( $extra_message ) ) {
            $message = sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s of Charitable! Use %3$s instead.', 'charitable' ), $function, $version, $extra_message );
        }
        else {
