	...
	abstract class Charitable_Abstract_Donation {
		...
		public function update_donation_log( $message, $deprecated_message = null ) {
			if ( is_int( $message ) ) {
				...
				$notice = '$donation_id no longer required as get_donation_log() is used in object context. Use $donation->update_donation_log($message)';

				charitable_get_deprecated()->deprecated_argument( __METHOD__, '1.3.0', $notice );
		...
		public function get_donation_log( $donation_id = null ) {
			if ( $donation_id ) {
				...
				$notice = '$donation_id no longer required as get_donation_log() is used in object context. Use $donation->get_donation_log()';
		...
		public function update_status( $new_status ) {
			...
			if ( false === charitable_is_valid_donation_status( $new_status ) ) {
				...
				if ( false === $new_status ) {
					charitable_get_deprecated()->deprecated_function( __METHOD__, sprintf( '%s is not a valid donation status.', $new_status ), '1.0.0' );
					return 0;
				}
			}
		...
		public function is_valid_donation_status( $status ) {
			charitable_get_deprecated()->deprecated_function( __METHOD__, '1.4.0', 'charitable_is_valid_donation_status( $status )' );
		...
		public function is_approved_status( $status ) {
			charitable_get_deprecated()->deprecated_function( __METHOD__, '1.4.0', 'charitable_is_approved_status( $status )' );
		...
		public function flush_campaigns_donation_cache( $donation_id ) {
			charitable_get_deprecated()->deprecated_function( __METHOD__, '1.4.0', 'charitable_sanitize_donation_meta( $donation_id )' );
