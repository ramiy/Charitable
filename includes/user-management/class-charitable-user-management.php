...
if ( ! class_exists( 'Charitable_User_Management' ) ) :
  ...
  class Charitable_User_Management {
    ...
    private static function user_has_only_ok_roles( $ok_roles, $not_ok_roles ) {
      ...
      if ( in_array( 'admintrator', $roles ) ) {
        return true;
      }

      $approved_roles = array_intersect( $ok_roles, $roles );
      $not_approved_roles = array_intersect( $not_ok_roles, $roles );

      if ( empty( $approved_roles )  || ! empty( $not_approved_roles ) ) {
        return false;
      }

      return true;

      // $ret = in_array( 'administrator', $roles ) || (
