...
function ed_late_init() {
	echo '<pre>'; 

	$currency_helper = charitable()->get_currency_helper();

	$amounts = array(
		'10.50', '10,50', '10000', '10,000', '100,00', 6.000, '12,500.50', '12.500,50'
	);

	foreach ( $amounts as $amount ) {
		printf( 'Sanitizing %s -> %s %s', $amount, $currency_helper->sanitize_monetary_amount( $amount ), PHP_EOL );
		print PHP_EOL;
	}
	
	printf( '<strong>Swapping decimal & thousands separators %s</strong>', PHP_EOL );
	...
	foreach ( $amounts as $amount ) {
		printf( 'Sanitizing %s -> %d %s', $amount, $currency_helper->sanitize_monetary_amount( $amount ), PHP_EOL );
		print PHP_EOL;		
	}	
...
// add_action( 'init', 'ed_late_init', 1000 );